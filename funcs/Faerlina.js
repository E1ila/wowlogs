...
module.exports = {
   ...
   processEvent: function (log, options, lineNumber, event, lastEvent, currentEncounter) {
      ...
      if (event.event === 'ENCOUNTER_START' && event.encounterId === 1110) {
         result.printPretty = true;
      }
      ...
      if (!currentEncounter || currentEncounter.encounterId !== 1110)
         return result;
      ...
      if (
         ...
         if (event.event === 'SPELL_AURA_REMOVED')
            ...
            result.extraText = `${c.purpleBg}✅${c.off}`;
         result.printPretty = true;
      }
      ...
      if (event.spell && event.spell.id === DISRUPTING_SHOUT && (!event.target || event.target.name !== 'Battle Chicken') && !slim) {
         ...
         && (!event.spell || event.spell.id !== RAIN_OF_FIRE)
         && (!event.absorbedSpell)
         && !slim
      ) {
